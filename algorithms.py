...
import collections
from neighbours import get_neighbours, get_key_value_neighbours, get_a_search_neighbours, get_a_search_neighbourssecond
...
def badastar_search(start_x, start_y, end_x, end_y):
    start = Point(start_x, start_y)
    end = Point(end_x, end_y)
    ...
    while not queue.empty():
        ...
        for neighbour in get_a_search_neighbourssecond(vertex, speed):
            if not contains_point(neighbour[1], visited):
                visited.append(neighbour[1])
                if neighbour not in (x for x in queue.queue):
                    queue.put((neighbour[0], neighbour[1]))
